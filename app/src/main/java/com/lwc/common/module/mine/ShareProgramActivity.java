...
public class ShareProgramActivity extends BaseActivity implements OnClickListener {
	...
	private void sharedToQQ() {
		// 添加QQ在分享列表页中
		new ShareAction(ShareProgramActivity.this)
...
//		}
	...
	private UMShareListener umShareListener = new UMShareListener() {
		...
		public void onResult(SHARE_MEDIA platform) {
			...
			ToastUtil.showLongToast(ShareProgramActivity.this, platform + " 分享成功啦");
			if(isOrderShare){
				isOrderShare = false;
				onOrderShared();
			}
		...
		public void onError(SHARE_MEDIA platform, Throwable t) {
			ToastUtil.showLongToast(ShareProgramActivity.this,platform + " 分享失败啦");
		}

		@Override
		public void onCancel(SHARE_MEDIA platform) {
			ToastUtil.showLongToast(ShareProgramActivity.this,platform + " 分享取消了");
		}
	};
	...
	private void onOrderShared() {
		HttpRequestUtils.httpRequest(this, "订单分享成功!", RequestValue.GET_ORDER_ORDERSHARE+"?orderId="+orderId, null, "GET", new HttpRequestUtils.ResponseListener() {
			...
			public void getResponseData(String response) {
				//finish();
				ToastUtil.showToast(ShareProgramActivity.this,"订单分享提交成功了！");
				String isShare = (String) SharedPreferencesUtils.getParam(ShareProgramActivity.this,"isShare","0");
				if("0".equals(isShare)){
					SharedPreferencesUtils.setParam(ShareProgramActivity.this,"isShare","1");
					checkedActivity();
				}
						...
						if (activityBeans != null && activityBeans.size() > 0) {
							...
							for (int i=0; i<activityBeans.size();i++) {
								...
								if("/order/orderShare".equals(activityBean.getConditionIndex())){
									ToastUtil.showToast(ShareProgramActivity.this,"找到活动了！");
									Bundle bundle = new Bundle();
									bundle.putSerializable("activityBean",activityBean);
									bundle.putString("activityId",activityBean.getActivityId());
									IntentUtil.gotoActivity(ShareProgramActivity.this,RedPacketActivity.class,bundle);
								}
							}
						}else{
							ToastUtil.showToast(ShareProgramActivity.this,"没有合适的活动！");
						}
						break;
					default:
						ToastUtil.showToast(ShareProgramActivity.this,"获取活动失败了！");
						break;
				}
			}
			@Override
			public void returnException(Exception e, String msg) {
				ToastUtil.showToast(ShareProgramActivity.this,"获取活动失败了！"+msg);
			}
		});
	}
