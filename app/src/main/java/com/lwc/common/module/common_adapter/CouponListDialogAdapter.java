 ...
 * 优惠券
 */
public class CouponListDialogAdapter extends SuperAdapter<Coupon> {

    private Context context;
    public CouponListDialogAdapter(Context context, List<Coupon> items, int layoutResId) {
    ...
    public void onBind(SuperViewHolder holder, int viewType, int layoutPosition, Coupon item) {
        ...
        txtDiscountAmount.setText(Utils.getSpannableStringBuilder(1, str.length(), context.getResources().getColor(R.color.white), str, 25));
 /*       if (item.getCouponType() == 1) {
        ...
        try {
            ...
            long time = item.getValidTime()*24*60*60*1000;
            holder.setText(R.id.txtTime, sdf2.format(date)+" - "+ sdf2.format(new Date(date.getTime()+time)));
