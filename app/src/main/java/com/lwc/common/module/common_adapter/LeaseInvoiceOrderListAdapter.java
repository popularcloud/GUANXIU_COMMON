 ...
 * 我的订单  发票
 */
public class LeaseInvoiceOrderListAdapter extends SuperAdapter<LeaseInvoiceOrder> {

    public LeaseInvoiceOrderListAdapter(Context context, List<LeaseInvoiceOrder> items, int layoutResId) {
        super(context, items, layoutResId);
    }

    @Override
    public void onBind(SuperViewHolder holder, int viewType, int layoutPosition, LeaseInvoiceOrder item) {
    ...
    /*    String type = item.getDeviceTypeName();
        ...
        holder.setText(R.id.tv_type, item.getGoodsName()); //设备类型
        String str = Utils.getMoney(Utils.chu(item.getPayPrice(),"100"))+" 元";
