 ...
 * 订单开票
 */
public class LeaseInvoiceFragment extends BaseFragment {
    ...
    private ArrayList<LeaseInvoiceOrder> myOrders = new ArrayList<>();
            ...
            case R.id.tv_next:
                ...
                for (int i=0; i<isSelect.size(); i++) {
                    if (isSelect.get(i).equals("1")) {
                        allManey = Utils.jia(allManey, myOrders.get(i).getPayPrice());
                        ids = ids+myOrders.get(i).getId()+",";
                    }
                }
                ...
                bundle.putString("totalManey", Utils.chu(allManey,"100"));
                bundle.putString("invoiceOperationType","leaseOrder");
    ...
    private void getOrderList() {
        ...
        HttpRequestUtils.httpRequest(getActivity(), "租赁开票", RequestValue.GET_INVOICE_LEASEORDER_LIST, params, "GET", new HttpRequestUtils.ResponseListener() {
            @Override
...
//					tv_all_select.setText("全选（"+myOrders.size()+"）");
    ...
    private void updateMoney() {
        ...
        String str = "合计: ¥ "+Utils.getMoney(Utils.chu(allManey,"100"));
...
//		recyclerView.setLayoutManager(new VegaLayoutManager(this));
        adapter = new LeaseInvoiceOrderListAdapter(getContext(), myOrders, R.layout.item_invoice_order);
