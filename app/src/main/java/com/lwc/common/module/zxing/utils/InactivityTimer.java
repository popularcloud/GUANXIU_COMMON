...
package com.lwc.common.module.zxing.utils;
...
public class InactivityTimer {
	...
	public synchronized void onPause() {
		cancel();
		if (registered) {
			LocalBroadcastManager.getInstance(activity).unregisterReceiver(powerStatusReceiver);
			registered = false;
		} else {
	...
	public synchronized void onResume() {
		if (registered) {
			...
			LocalBroadcastManager.getInstance(activity).registerReceiver(powerStatusReceiver, new IntentFilter(Intent.ACTION_BATTERY_CHANGED));
			registered = true;
		}
		onActivity();
	}
